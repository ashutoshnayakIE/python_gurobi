...
request = [0 for t in range(48)]
requested = [0 for t in range(48)]
incomplete = [0 for t in range(48)]
objVal = []
queue = []
drift = []
penalty = []
...
for t in range(48):
    ...
    for k in range(min(5,len(plT[t]))):
        requested[t] += 1*(Qf>0)
        request[t] += 1
    ...
    for f in range(nf):
        ...
        for k in range(len(plT[t])):
            ...
            if tempflag == 0 and t < order[k][0]: 
                plT[t+1].append(plT[t][k])
                incomplete[t] += 1*(Qf>0)
            
            elif  tempflag == 0 and t == order[k][0]:
                j = order[k][1]
                
                for t2 in range(plT[t][k][j][1]):
                    nsLoad[t+1+t2] += plT[t][k][j][0]
                    consumptionR[t+1+t2] += plT[t][k][j][0]
    ...
    costR += G.X*macroCost[t]/2+X.X*microCost[t]/2
    
    # printing the output values
    drift.append(realtime.ObjVal-V1*G.X*macroCost[t]/2-V1*X.X*microCost[t]/2)
    penalty.append(V1*G.X*macroCost[t]/2+V1*X.X*microCost[t]/2)
    objVal.append(realtime.ObjVal)
    queue.append(sum(Qr)+sum(Qc)+Qf)
    print t,drift[-1],penalty[-1],objVal[-1],sum(Qr)/nr,sum(Qc)/nc,Qf,nsLoad[t]
    
    
cost[0] += costR

print cost[0],len(objVal)

plt.figure(1)

plt.subplot(231)
plt.plot(incomplete,label='incomplete')
plt.plot(requested,label='requested')
plt.plot(request,label='request')
plt.legend()

plt.subplot(232)
plt.plot(consumptionR,label = 'consumption')
plt.legend()
